...
This script also includes an additional camera feature as a seperate thread to capture images from the vehicle's perspective. Due to the additional processing demand
this can result in the PID controller missing the target waypoint. The camera feed is displayed in a seperate window and can be closed by pressing 'q'. Modify
sleep timers and PID tuners as needed if the vehicle is missing waypoints.
...
# start_point = spawn_points[0] # Use the first spawn point
start_point = carla.Transform(carla.Location(x=-20.19, y=137.09, z=0.00), carla.Rotation(yaw=0))
...
try:
    ...
    draw_circle(world, start_point.location)
    draw_circle(world, end_location, color=carla.Color(r=0, g=200, b=125))
    ...
    while not reached_target:
        ...
        world.debug.draw_point(target_location, size=0.1, color=carla.Color(r=255, g=0, b=0), life_time=0.1)
