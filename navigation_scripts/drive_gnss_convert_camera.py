...
This script also utilizes an OpenCV camera feed to display the camera data in a separate window using a separate thread
Camera can be closed by pressing 'q'.
 '''
...
world_map = world.get_map()  # Add this line to get the map
    ...
    try:
        while True:
            if gnss_data["latitude"] is not None and gnss_data["longitude"] is not None:
                ...
                ref_geolocation = world_map.transform_to_geolocation(ref_location)
