...
# Function to generate uniformly spaced waypoints
def generate_uniform_waypoints(route, distance_interval):
    waypoints = []
    distance_accumulated = 0.0

    for i in range(len(route) - 1):
        current_wp = route[i][0].transform.location
        next_wp = route[i + 1][0].transform.location
        segment_length = calculate_distance(current_wp, next_wp)

        while distance_accumulated + distance_interval <= segment_length:
            ratio = (distance_accumulated + distance_interval) / segment_length
            interpolated_wp = interpolate_location(current_wp, next_wp, ratio)
            waypoints.append(interpolated_wp)
            distance_accumulated += distance_interval

        distance_accumulated -= segment_length

    waypoints.append(route[-1][0].transform.location)
    return waypoints

def interpolate_location(start_loc, end_loc, ratio):
    x = start_loc.x + (end_loc.x - start_loc.x) * ratio
    y = start_loc.y + (end_loc.y - start_loc.y) * ratio
    z = start_loc.z + (end_loc.z - start_loc.z) * ratio
    return carla.Location(x=x, y=y, z=z)



# Generate uniformly spaced waypoints
distance_interval = 2.0  # Distance between waypoints in meters
uniform_route = generate_uniform_waypoints(route, distance_interval)
...
for i, waypoint in enumerate(uniform_route):
    print(f"Waypoint {i}: Location({waypoint.x}, {waypoint.y}, {waypoint.z})")
...
try:
    ...
    while not reached_target:
        ...
        target_location = uniform_route[target_index]
        ...
        if distance < 0.9:  # Move to the next waypoint if close enough
            ...
            target_index += 1
            if target_index >= len(uniform_route):
                reached_target = True
                continue
            target_location = uniform_route[target_index]
        ...
        target_yaw = math.atan2(target_location.y - vehicle_location.y, target_location.x - vehicle_location.x)
