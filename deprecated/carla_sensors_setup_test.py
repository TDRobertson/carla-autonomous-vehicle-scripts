# Description: This script demonstrates how to combine various sensors and image processing techniques to create a simple autonomous driving simulation in the Carla Simulator.
# The script uses the CARLA Python API to interact with the simulator and subscribe to sensor data such as RGB camera images, GNSS data, IMU data, collision events, lane invasion events, and obstacle detection events.
# The script also uses the Global Route Planner to create a route between two waypoints and moves the vehicle along the route while collecting sensor data.
...
def move_vehicle_to_next_waypoint(vehicle, waypoint):
    ...
    control.steer = np.clip(angle, -1.0, 1.0)
