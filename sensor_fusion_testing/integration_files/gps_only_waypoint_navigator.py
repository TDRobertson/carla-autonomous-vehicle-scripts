...
class GPSOnlyWaypointNavigator:
    """
    GPS-only waypoint navigation system without sensor fusion.
    Uses only GPS positioning for navigation, showing raw effects of GPS spoofing.
    """
    
    def __init__(self, 
                 vehicle: carla.Vehicle,
                 gps_system: Optional[GPSOnlySystem] = None,
        ...
        """
        Initialize the GPS-only waypoint navigator.
        
        Args:
            vehicle: CARLA vehicle actor
            gps_system: Optional GPSOnlySystem instance (will create one if None)
        ...
        # Initialize GPS-only system if not provided
        if gps_system is None:
            self.gps_system = GPSOnlySystem(
                vehicle, 
    ...
    def set_waypoints(self, waypoints: List[carla.Waypoint]):
        ...
        self.current_waypoint_index = 0
        print(f"Set {len(waypoints)} waypoints for GPS-only navigation")
    ...
    def get_position_estimate(self) -> Optional[np.ndarray]:
        """Get the current GPS position estimate"""
        if self.gps_system:
            position = self.gps_system.get_fused_position()
            # Fallback to vehicle transform if GPS returns None
            if position is None:
    ...
    def get_true_position(self) -> Optional[np.ndarray]:
        ...
        if self.gps_system:
            return self.gps_system.get_true_position()
        else:
            # Fallback to vehicle transform
            transform = self.vehicle.get_transform()
            return np.array([transform.location.x, transform.location.y, transform.location.z])
    ...
    def calculate_distance_to_waypoint(self, waypoint: carla.Waypoint) -> float:
        """Calculate distance from current GPS position to waypoint"""
    ...
    def update_navigation_stats(self, dt: float):
        """Update navigation statistics including GPS error tracking"""
        ...
        # Update GPS error statistics
        if self.gps_system:
            position_error = self.gps_system.get_position_error()
            self.navigation_stats['total_position_error'] += position_error
            self.navigation_stats['max_position_error'] = max(self.navigation_stats['max_position_error'], position_error)
            
            if elapsed_time > 0:
                self.navigation_stats['average_position_error'] = self.navigation_stats['total_position_error'] / (elapsed_time * 10)  # Assuming 10Hz updates
    ...
    def navigate_step(self, dt: float) -> Dict[str, Any]:
        """
        Perform one navigation step using GPS-only positioning.
        ...
        position_error = self.gps_system.get_position_error() if self.gps_system else 0.0
        
        # Check if waypoint reached
        if distance_to_waypoint < self.waypoint_reach_distance:
            self.current_waypoint_index += 1
            self.navigation_stats['waypoints_reached'] += 1
            print(f"Reached waypoint {self.current_waypoint_index - 1}/{len(self.waypoints)} (GPS-only)")
    ...
    def start_navigation(self):
        """Start the GPS-only navigation process"""
        if not self.waypoints:
            print("No waypoints set for GPS-only navigation")
            return False
        ...
        print(f"Started GPS-only navigation with {len(self.waypoints)} waypoints")
        return True
    ...
    def stop_navigation(self):
        ...
        print("GPS-only navigation stopped")
        
    def get_navigation_stats(self) -> Dict[str, Any]:
        """Get current navigation statistics including GPS error metrics"""
    ...
    def get_gps_system_stats(self) -> Dict[str, Any]:
        ...
        if self.gps_system:
            return {
                ...
                'position_error': self.gps_system.get_position_error(),
                'velocity_error': self.gps_system.get_velocity_error(),
                'error_statistics': self.gps_system.get_error_statistics()
            }
        return {}
